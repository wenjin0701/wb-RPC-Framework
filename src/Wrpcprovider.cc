#include "Wrpcprovider.h"
#include "Wrpcapplication.h"
#include "Wrpcheader.pb.h"
#include "WrpcLogger.h"
#include <iostream>



// 注册服务对象及其方法，以便服务端能够处理客户端的RPC请求
void WrpcProvider::NotifyService(google::protobuf::Service *service) {
...
void WrpcProvider::Run() {
    // 读取配置文件中的RPC服务器IP和端口
    std::string ip = WrpcApplication::GetInstance().GetConfig().Load("rpcserverip");
    int port = atoi(WrpcApplication::GetInstance().GetConfig().Load("rpcserverport").c_str());

    // 使用muduo网络库，创建地址对象
    muduo::net::InetAddress address(ip, port);

    // 创建TcpServer对象
    std::shared_ptr<muduo::net::TcpServer> server = std::make_shared<muduo::net::TcpServer>(&event_loop, address, "WrpcProvider");

    // 绑定连接回调和消息回调，分离网络连接业务和消息处理业务
    server->setConnectionCallback(std::bind(&WrpcProvider::OnConnection, this, std::placeholders::_1));
    server->setMessageCallback(std::bind(&WrpcProvider::OnMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
// 连接回调函数，处理客户端连接事件
void WrpcProvider::OnConnection(const muduo::net::TcpConnectionPtr &conn) {
    if (!conn->connected()) {
        // 如果连接关闭，则断开连接
        conn->shutdown();
    }
...
void WrpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp receive_time) {
    ...
    while (buffer->readableBytes() >= 4) {
        ...
        // 4. 业务逻辑处理
        if (!wrpcHeader.ParseFromString(rpc_header_str)) {
            std::cout << "header parse error" << std::endl;
            return;
        }
        
        std::string service_name = wrpcHeader.service_name();
        std::string method_name = wrpcHeader.method_name();
        ...
        google::protobuf::Closure *done = google::protobuf::NewCallback<WrpcProvider,
                                                                        const muduo::net::TcpConnectionPtr &,
                                                                        google::protobuf::Message *>(this,
                                                                                                     &WrpcProvider::SendRpcResponse,
                                                                                                     conn, response);
...
// 发送RPC响应给客户端
void WrpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr &conn, google::protobuf::Message *response) {
...
// 析构函数，退出事件循环
WrpcProvider::~WrpcProvider() {
    std::cout << "~WrpcProvider()" << std::endl;
