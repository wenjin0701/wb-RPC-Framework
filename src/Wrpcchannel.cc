...
ssize_t WrpcChannel::recv_exact(int fd, char* buf, size_t size) {
    size_t total_read = 0;
                             ...
                             ::google::protobuf::Closure *done)
{
    ...
    wrpcHeader.set_service_name(service_name);
    wrpcHeader.set_method_name(method_name);
    wrpcHeader.set_args_size(args_str.size());

    std::string rpc_header_str;
    if (!wrpcHeader.SerializeToString(&rpc_header_str)) {
...
// 创建新的socket连接
bool WrpcChannel::newConnect(const char *ip, uint16_t port) {
    // 创建socket
...
// 从ZooKeeper查询服务地址
std::string WrpcChannel::QueryServiceHost(ZkClient *zkclient, std::string service_name, std::string method_name, int &idx) {
...
// 构造函数，支持延迟连接
WrpcChannel::WrpcChannel(bool connectNow) : m_clientfd(-1), m_idx(0) {
