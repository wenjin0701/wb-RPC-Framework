...
Wrpcconfig WrpcApplication::m_config;  // 全局配置对象
std::mutex WrpcApplication::m_mutex;  // 用于线程安全的互斥锁
WrpcApplication* WrpcApplication::m_application = nullptr;  // 单例对象指针，初始为空
...
WrpcApplication &WrpcApplication::GetInstance() {
    ...
    if (m_application == nullptr) {  // 如果单例对象还未创建
        m_application = new WrpcApplication();  // 创建单例对象
...
void WrpcApplication::deleteInstance() {
